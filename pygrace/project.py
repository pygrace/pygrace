...
class Project(GraceObject):
    ...
    def saveall(self, filname='temp.agr'):
        """save the Project to a xmgrace project (.agr) file"""
        return self.write_agr(filname)
    ...
    def printfile(self, filename='temp.eps', filetype=None):
        """Use gracebat to output a image file of the specified type.
        The allowed types are:

            X11 PostScript EPS MIF SVG PNM JPEG PNG Metafile
        """
        return self.write_file(filename, filetype)
    ...
    def write_file(self, filename='temp.eps', filetype=None):
        ...
        if ext=="agr" or (filetype and filetype.lower() == 'agr'):
            ...
            message = """
The gracebat filetype is implied by the given filename. However, the
filetype '%s' was not recognized. Please specify the gracebat filetype
...
"""
        ...
        # all graphs
        xmins,ymins,xmaxs,ymaxs = list(zip(*worlds))
        xmin = min(xmins)
        xmax = max(xmaxs)
        for graph in graphs:
...
"""
    ...
    def autoscale_same(self,padx=0,pady=0, graphs=(), exclude_graphs=()):
        """Autoscale all graphs in a this MultiPlotProject to have the same x,y
        world coordinates.
        """
    ...
    def set_world_same(self,xmin,ymin,xmax,ymax):
        """Rescale all graphs in a MultiPlotProject to have the same x,y world
        coordinates.
        """
        for graph in self.graphs:
